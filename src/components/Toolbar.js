...
`;
  ...
  const exportPNG = () => {
    ...
    // 获取当前数学公式渲染器设置
    const mathRenderer = settings?.mathRenderer || 'none';
    
    // 如果使用MathJax，需要等待MathJax完成渲染
    const waitForMathRendering = () => {
      return new Promise((resolve) => {
        if (mathRenderer === 'mathjax' && window.MathJax) {
          window.MathJax.typesetPromise([clone]).then(() => {
            // 给MathJax一些额外时间完成渲染
